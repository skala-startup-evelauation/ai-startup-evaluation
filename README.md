...
```bash
[📊 시장성 평가 에이전트] 
      | 사용자가 질문 던지면: 최근 투자 규모나 성장률이 높은 AI 분야는? -> 벡터DB에 저장된 문서 기반으로 답변 생성(Multi-Query Retriever)
      V
[🔍 스타트업 탐색 에이전트]
      |
      |— Loop: 유망 기업 필터링 (최대 5개 AI 관련 스타트업)
      V
[🗜️ 기술 요약 에이전트]
      |
      |— Branch:
...
[👤 창업자 평가 에이전트]
      ...
      |    └── 없음 → "○○ 인터뷰" 등 보조 검색
      V
[🥊 경쟁사 비교 에이전트]  ← (기술 정보 + 시장 카테고리 기반 경쟁사 탐색)
      |
      V
[🧮 투자 판단 에이전트]
      |
      |— 내부 스코어링
      |     Owner (30%), Opportunity (25%), Product (15%) ...
      |— 리스크 패널티 적용
      |— 최종 투자 등급 부여: "투자" / "보류" / "제외"
      V
[📝 보고서 생성 에이전트]
      |
      |— 각 항목 통합 + 자연어 요약 + PDF 자동 생성
      |— 스타일 지정 (폰트, 테이블, 컬러 등)
      V
   [✅ 최종 보고서 출력]
...
| **카테고리**         | **문서 예시 (한국 기준)**                                                                                                                  | **목적**                      |
|----------------------|---------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------|
| 산업/시장 리포트      | 1. `RE169_국내AI창업기업_비즈니스_현황분석` (3장, 5장만 파싱)  <br> 2. `IF_Strategy24-03_글로벌_정부·민간_분야_AI_투자_동향_분석_최종` (11페이지부터, 참고문헌 제외) <br> 3. `250130__startup-recipe_investment-report_24_v00190` (전체) | 시장 규모/성장성 분석       |
| 산업 뉴스/인사이트   | `벤처캐피탈 자율규제 우수기업 평가기준`                                                                                                   | 시장 트렌드 및 경쟁사 정보   |
