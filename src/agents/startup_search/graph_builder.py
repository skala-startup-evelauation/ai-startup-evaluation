...
def build_graph() -> StateGraph[StartupSearchState]:
    builder = StateGraph(StartupSearchState)

    # 1) 엑셀에서 링크 읽기
    def read_links(state: StartupSearchState) -> NodeOutput:
        state.companies = get_company_links_by_category(state.sector_keyword)
        return state

    # 2) 비동기 LLM+브라우저 처리 → AI 기업만 필터링
    async def fetch_and_filter(state: StartupSearchState) -> NodeOutput:
