...
def search_background_info(startup_name: str) -> List[str]:
    ...
    return [
        f"{startup_name} 스타트업에 대한 분석을 시작합니다."
    ]
...
#         founder_name = ask_gpt(f"{startup_name}의 창업자 이름은 누구인가요?")
    ...
    # 이름이 없으면 GPT로 추론, 있으면 그대로 사용
    if not founder_name:
        raw_response = ask_gpt(
            f"{startup_name}의 창업자 이름만 정확히 알려주세요. 설명 없이 이름만 주세요.",
            system_msg=f"{background_summary}\n이 정보를 참고해서 창업자의 정확한 '이름만' 출력하세요. 예: 김봉진"
        )
        # 이름 파싱
        founder_name = raw_response.strip().split("은")[0].split("는")[0].strip(" ,입니다.")
        print(f"[디버깅] 추론된 창업자 이름: {founder_name}")
    else:
        print(f"[디버깅] 수동 지정된 창업자 이름: {founder_name}")
